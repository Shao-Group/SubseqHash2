...
{    
	...
	ifstream table(argv[6]);

	string refseq, info, t;

	while(getline(table, info))
		t = info;
	...
	for(int i = 0; i < subsample; i++)
	{
		for(int j = 0; j < refnum; j++)
		{
			string refpath = "./refsub2/" + species + "/" + refname[j] + "_" + to_string(n) + "_" + to_string(k) + "_" + to_string(d) + "_" + to_string(tablenum[i]);
		...
		string qpath = "./readsub2/" + species + "/" + to_string(n) + "_" + to_string(k) + "_" + to_string(d) + "_" + to_string(tablenum[i]);
		string qpath_rc = "./readsub2/" + species + "/" + to_string(n) + "_" + to_string(k) + "_" + to_string(d) + "_" + to_string(tablenum[i]^1);
