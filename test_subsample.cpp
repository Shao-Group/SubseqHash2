...
{ 
    ...
    int64_t threshold = atoll(argv[6]);

    dim1 = (n+1) * (k+1) * d;

    subseqhash2seeding sub2(n, k, d, subsample, threshold);
    sub2.init(argv[7]);
	...
	printf("%d/%d/%d/%d/%lld, %.2lf, %.2lf, %.4lf, %.4lf, %.4lf, %.4lf, %.2lf, %.4lf, %.2lf, %.2lf\n", n, k, d, subsample, threshold,
