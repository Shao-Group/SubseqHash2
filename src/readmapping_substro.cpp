...
void pseudo_match(int len, int readnum, vector<seed> &seedt, vector<int> &align, int strand)
{
	matches.clear();
	...
	for(seedmatch m: matches)
	{
		...
		if(2 * tp >= k + prek)
		{
			truematches++;	
			if(strand)
				bcover[readnum][(len - m.s2->st - 1)/200] = 1;
			else
...
{    
	...
	{
		string refpath = "./refseed_substro/" + to_string(n) + "_" + to_string(k) + "_" + to_string(d) + "_" + to_string(i) + "_" + to_string(prek);
		seeds.clear();
		loadSeedsStr(refpath.c_str(), seeds, k);
		refindex = index_build(seeds);
	
		string qpath = "./readseed_substro/" + to_string(n) + "_" + to_string(k) + "_" + to_string(d) + "_" + to_string(i) + "_" + to_string(prek);
		...
		for(int j = 0; j < num; j++)
		{
			...
			pseudo_match(reads[j], j, seedt, aligned[j], 0);
			...
			pseudo_match(reads[j], j, seedt, aligned[j], 1);
		}
	}
	...
	printf("%d/%d/%d/%d/%d, %d, %d, %.4lf, %.4lf, %d, %d, %.4lf\n", n, k, d, subsample, prek, int(ans[0]), int(ans[1]), ans[1]/ans[0], ans[2] / ans[10], int(ans[3]), int(ans[4]), ans[4]/ans[3]);
