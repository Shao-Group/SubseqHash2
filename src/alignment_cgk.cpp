...
void pseudo_match(string s, string t, vector<int> &align)
{
	...
	for(int r = 0; r < repeat; r++)
	{
		...
		start = clock();
		seedings[r].get_cgk(s, seeds);
		seedings[r].get_cgk(t, seedt);
		end = clock();
...
{    
    if(argc != 4)
    {
		printf("usage: alignment_cgk.out readFile k repeat\n");
		return 1;
    }
    ...
    for(int i = 1; i <= repeat; i++)
    	seedings.push_back(cgkseeding(k));
