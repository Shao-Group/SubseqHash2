...
void pseudo_match(string s, vector<int> &align, minimizerseeding seeding, int no)
{
	int lens = s.length();
...
{    
	...
	seeding.get_minimizers(refseq, seedu);
	...
	printf("%d/%d/%d, %d, %d, %.4lf, %.4lf, %d, %d, %.4lf\n", k+w-1, k, w, int(ans[0]), int(ans[1]), ans[1]/ans[0], ans[2] / ans[10], int(ans[3]), int(ans[4]), ans[4]/ans[3]);
    return 0;
}