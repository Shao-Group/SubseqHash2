...
void pseudo_match(string s, string t, vector<int> &align, syncmerseeding seeding)
{
	int lens = s.length();
	int lent = t.length();
	...
	for(seedmatch m: matches)
	{
		...
		{	
			...
			// cout<<s.substr(m.s1->st, k)<<endl;
			// cout<<t.substr(m.s2->st, k)<<endl;
			// cout<<(uint64_t)m.s1->hashval<<" "<<(uint64_t)m.s2->hashval<<endl;
			for(int i = m.s1->st; i <= m.s1->ed; i++)
				fscover[i] = 1;
			for(int i = m.s2->st; i <= m.s2->ed; i++)
				fscover2[i] = 1;
...
{    
	...
	printf("%d/%d, %.2lf, %.2lf, %.4lf, %.4lf, %.4lf, %.4lf, %.2lf, %.4lf, %.2lf, %.2lf\n", k, s,
