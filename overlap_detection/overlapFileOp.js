...
/*
  Given two overlap results, compute their intersection or union.

  In the files, each line starts with the two labels of the overlapping pair. The files are assumed to
...
function processOneLine(fin, buf){
	...
	result.p = result.s.split(" ").map(function(x){if(x.indexOf(",")>-1){return x;}else{return parseInt(x)}});
    }
    return result;
}
...
function main(args)
{
	...
	if(l1.p[0] > l2.p[0]){
		...
		if(do_union){
			if(l1.p.length > 3){//has freq info
				cur_min = l1.p[3] < l2.p[3] ? l1.p[3] : l2.p[3];
				cur_max = l1.p[4] > l2.p[4] ? l1.p[4] : l2.p[4];
				print([l1.p[0], l1.p[1], l1.p[2] + l2.p[2], cur_min, cur_max].join(" "));
			}else{
				print([l1.p[0], l1.p[1], l1.p[2] + l2.p[2]].join(" "));
			}
		}else{
		    print([l1.p[0], l1.p[1], l1.p[2], l2.p.slice(2)].join(" "));
		}
		l1 = processOneLine(fin1, buf);
		l2 = processOneLine(fin2, buf);
	    }
